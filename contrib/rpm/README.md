...
The RPM spec file provided here is for Mazacoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://github.com/mazaska/mazacoin/releases and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Mazacoin git repository](https://github.com/mazaska/mazacoin) but are not part of
...
Currently only `Source10` is used, for the example `mazacoin.conf` file.
...
in the mazacoin git repository. At present only an SVG version of the Mazacoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the mazacoin
...
needed when 0.12.1 is released, a proper fix is already in the Mazacoin git
master branch.
...
idea to build Mazacoin Core against a static version of that library compiled
...
The `mazacoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `mazacoin-server` package. At this point, most RPM based
...
The `mazacoin-server` package should have SELinux support. How to properly do
...
Mazacoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
4. I renamed the `mazacoin` package that contains the Qt GUI to `mazacoin-core` as
that appears to be how the general population refers to it, in contrast to
`mazacoin-xt` or `mazacoin-classic`. I wanted to make sure the general population
